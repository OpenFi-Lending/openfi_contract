...
interface IL2Pool {
  ...
  /**
   * @notice Calldata efficient wrapper of the repayWithBTokens function
   * @param args Arguments for the repayWithBTokens function packed in one bytes32
   *    104 bits             8 bits               128 bits       16 bits
   * | 0-padding | shortenedInterestRateMode | shortenedAmount | assetId |
   * @dev the shortenedAmount is cast to 256 bits at decode time, if type(uint128).max the value will be expanded to
   * type(uint256).max
   ...
   * @return The final amount repaid
   */
  function repayWithBTokens(bytes32 args) external returns (uint256);
   ...
   * | 0-padding | receiveBToken | shortenedDebtToCover |
